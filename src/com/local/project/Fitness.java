...
public class Fitness {
    ...
    protected void enterTheZone(Card[] zone, Card card) {
        // В жизни думаю логичнее сначала проверить на время посещения, т.к. зал может быть вообще зарыт,
        ...
        for (int i = 0; i < allZones.length; i++) {
            for (int j = 0; j < allZones[i].length; j++) {
                if (allZones[i][j] == card) {
                    System.out.printf("Абонемент љ%d уже зарегистрирован в зоне \"%s\" \n",
    ...
    private void printInfoOneCard(Card[] zone, Card card) {
        System.out.printf("Абонемент љ%d: %s %s, посещаемая зона - %s\n", card.getNUMBER_OF_CARD(),
